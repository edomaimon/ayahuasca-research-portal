...
export async function getFeaturedArticles() {
  try {
    ...
    console.error('getFeaturedArticles failed:', err.message);
    return [];
  }
}

/**
 * Get previous and next articles in the same category (sorted by year desc, then title).
 */
export async function getAdjacentArticles(article) {
  try {
    const categoryArticles = await getAllArticles({ category: article.category, sortBy: 'newest' });
    // Secondary sort by title for stable ordering within same year
    categoryArticles.sort((a, b) => {
      const yearDiff = b.year - a.year;
      return yearDiff !== 0 ? yearDiff : a.title.localeCompare(b.title);
    });
    const idx = categoryArticles.findIndex(a => a.slug === article.slug);
    return {
      prev: idx > 0 ? { title: categoryArticles[idx - 1].title, slug: categoryArticles[idx - 1].slug } : null,
      next: idx < categoryArticles.length - 1 ? { title: categoryArticles[idx + 1].title, slug: categoryArticles[idx + 1].slug } : null,
    };
  } catch {
...
/**
 * Get unique author count from all articles.
 */
export async function getUniqueAuthorsCount() {
  try {
    const articles = await getAllArticles();
    const authors = new Set();
    articles.forEach(a => (a.authors || []).forEach(name => authors.add(name)));
    return authors.size;
  } catch {
    return 0;
  }
